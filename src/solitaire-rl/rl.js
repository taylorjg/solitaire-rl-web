...
const makeModel = () => {
  const kernelInitializer = tf.initializers.randomUniform({
    minval: -0.5,
    maxval: +0.5,
  });
  ...
  model.add(
    tf.layers.dense({ units: 1, name: "output-layer", kernelInitializer })
  );

  const summaryLines = [];
  model.summary(undefined, undefined, (summaryLine) =>
...
const makeLinearDecaySchedule = (startVal, endVal, decayEpisodes) => {
  ...
  return (episode) =>
    episode > decayEpisodes ? endVal : startVal + increment * episode;
};
...
const checkEndCondition1 = (model) => {
  ...
  console.log(
    `[checkEndCondition1] actions (${actions.length}): ${JSON.stringify(actions)}; solved: ${solved}`
  );
  return solved;
};
...
const checkEndCondition2 = (model) => {
  ...
  return initialActions.every((initialAction) => {
    ...
    const solved = agent.solved;
    console.log(
      `[checkEndCondition2] actions (${actions.length}): ${JSON.stringify(actions)}; solved: ${solved}`
    );
    return solved;
  });
};
...
const trainLoop = async (env, model, pi, endCondition, callbacks) => {
  ...
  const epsilonDecaySchedule = makeLinearDecaySchedule(
    EPSILON_START,
    EPSILON_END,
...
export const makeRandomAgent = () => new RandomAgent();
