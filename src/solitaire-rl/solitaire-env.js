...
function* iterActions() {
  for (const fromLocation of LOCATIONS) {
    for (const direction of DIRECTIONS) {
      ...
      if (
        LOCATIONS_MAP.has(viaLocation.key) &&
        LOCATIONS_MAP.has(toLocation.key)
      ) {
...
export class Board {
  constructor(boardState) {
    if (boardState) {
      ...
      this._boardState = new Map(
        LOCATIONS.map((l) => [l.key, !l.sameAs(CENTRE)])
      );
    }
  }
  ...
  get solved() {
    const remainingPieceCount = Array.from(this._boardState.values()).filter(
  ...
  validActions() {
    ...
    return actionIndices.filter((actionIndex) =>
      this.isValidAction(actionIndex)
    );
  }
  ...
  entries() {
    return Array.from(this._boardState.entries()).map(
      ([locationKey, isOccupied]) => [
        LOCATIONS_MAP.get(locationKey),
        isOccupied,
      ]
    );
  }
